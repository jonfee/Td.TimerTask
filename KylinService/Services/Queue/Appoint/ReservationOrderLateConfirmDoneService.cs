...
namespace KylinService.Services.Queue.Appoint
{
    ...
    /// 预约订单超时未确认完成服务
    /// </summary>
    public sealed class ReservationOrderLateConfirmDoneService : QueueSchedulerService
    {
        /// <summary>
        /// 任务计划数据所在Redis配置
        /// </summary>
        ...
        public ReservationOrderLateConfirmDoneService(Form form, DelegateTool.WriteMessageDelegate writeDelegate) : base(QueueScheduleType.ReservationOrderLateConfirmDone, form, writeDelegate)
        {
            config = Startup.ScheduleRedisConfigs[QueueScheduleType.ReservationOrderLateConfirmDone];
        }
        ...
        {
            //获取一条待处理数据
            var model = null != config ? config.DataBase.ListLeftPop<ReservationOrderLateReceiveModel>(config.Key) : null;
            ...
            if (null != model)
            {
                ...
                if (model.OrderID.ToString() != Startup.DirtyDataPKValue)
                {
                    ...
                    //输出消息
                    string message = string.Format("预约服务订单(ID:{0})在{1}天{2}小时{3}分{4}秒后未确认服务完成系统将自动确认服务完成", model.OrderID, duetime.Days, duetime.Hours, duetime.Minutes, duetime.Seconds);
        ...
        {
            var model = new ReservationOrderLateReceiveModel();
